module.exports = {
  ...
  async initcloud() {
    ...
    const normalinfo = require('./env.js').envList || []; // 读取 env 文件
    if (shareinfo.envid != null) {
      // 如果 ext 配置文件存在，环境共享模式
      ...
      if (normalinfo.length != 0 && normalinfo[0].envId != null) {
        ...
        // 如果文件中 envlist 不存在，提示要配置环境
        this.cloud = () => {
          wx.showModal({
            content: '当前小程序没有配置云开发环境，请在 env.js 中配置你的云开发环境',
            showCancel: false,
          });
          throw new Error('当前小程序没有配置云开发环境，请在 env.js 中配置你的云开发环境');
        };
      }
    }
  },
  ...
  async getOpenId() {
    const {
      ...
      .catch((e) => {
        let flag = e.toString();
        flag =
          flag.indexOf('FunctionName') == -1
            ? flag
            : '请在cloudfunctions文件夹中getOpenId上右键，创建部署云端安装依赖，然后再次体验';
        wx.hideLoading();
        wx.showModal({
